...
- Backend output combines radius + direction into signed radius:
  CW (G02) → positive, CCW (G03) → negative

OD mode convention:
- CW (G02): arc curves away from operator (convex on part)
- CCW (G03): arc curves toward operator (concave on part)
"""
...
class SegmentListWidget(QWidget):
    ...
    def set_segments(self, segments: List[dict]):
        ...
        try:
            ...
            for seg in segments:
                ...
                direction = "CCW" if signed_radius < 0 else "CW"
                self._add_row(
    ...
    def _read_row(self, row: int) -> Optional[dict]:
        ...
        # Combine into signed radius: CW → positive, CCW → negative
