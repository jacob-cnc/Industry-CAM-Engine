...
class FinishPlanner:
    ...
    def plan(
        ...
        # Z0+fin for clearance approach
        # We need fin_allowance but it's not in finishing_params — use a small clearance
        # The G-code writer handles the rapid to (X_start, Z0+fin)
        # The first move feeds from Z0+fin to Z0, then traces the profile

        # Build moves: first feed from approach to profile start, then trace ALL segments
        moves = []

        # First move: feed from (X_start, Z0+fin) to profile start (segment[0])
        # This gets the tool from the approach position onto the profile
        ...
        # x_level = X_start (approach X), z_start = Z0+fin (approach Z for rapid)
        # The G-code writer uses these for the approach sequence
        fin_clearance = 0.001  # Small Z clearance above Z0
        finish_pass = TurningPass(
            x_level=stock.x_start,
            z_start=fin_clearance,
